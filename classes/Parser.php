...
class Parser {
    ...
    public static function parseRoutes(): array
    {
        ...
        $controllerFiles[$controllersPath] = scandir($controllersPath);
        foreach(getAllActiveModules() as $module) {
            ...
            if(file_exists(BASE_PATH . 'modules/' . $moduleName . '/controllers/')) {
                $controllerFiles[$moduleControllersPath] = scandir(BASE_PATH . 'modules/' . $moduleName . '/controllers/');
            }

        }
        foreach($controllerFiles as $controllersPath => $files){
